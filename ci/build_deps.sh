...
dward_SRC_PATH=${THIRD_PARTY_PATH}/dwarf
dward_BUILD_PATH=${OUTPUT_PATH}/dwarf

cd ${dward_SRC_PATH}
./ci/build_deps.sh
cd ${ROOT_PATH}
mkdir ${dward_BUILD_PATH}

cmake -S ${dward_SRC_PATH} -B ${dward_BUILD_PATH} \
  -DCMAKE_INSTALL_PREFIX=${INSTALL_PATH} \
  -DDWARD_BUILD_TEST=OFF \
  -DDWARD_BUILD_BENCHMARK=OFF \
  -DDWARD_BUILD_EXAMPLES=OFF

cmake --build ${dward_BUILD_PATH} -j 4
cmake --install ${dward_BUILD_PATH} --prefix ${INSTALL_PATH}
exit 0
